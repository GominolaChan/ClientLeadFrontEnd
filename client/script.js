...
async function getProjects() {
    ...
    try {
        ...
        const response = await fetch(urlBack + "/api/users/me?populate=*", requestOptions)
...
async function createProject(object) {
    ...
    try {
        ...
        const response = await fetch(urlBack + "/api/proyectos", requestOptions);
        if (!response.ok) {
...
async function updateProject() {
    ...
    try {
        ...
        const response = await fetch(urlBack + "/api/proyectos/" + idUpdate, requestOptions);
        if (!response.ok) {
            if (response.status == 400 || response.status == 401) {
                document.getElementById('formProjectContent').classList.remove('hidden')
                document.getElementById('loaderAddProject').classList.add('hidden')
...
async function deleteProject(id) {
    ...
    try {
        ...
        if (confirmation) {
            ...
            const response = await fetch(urlBack + "/api/proyectos/" + id, requestOptions)
